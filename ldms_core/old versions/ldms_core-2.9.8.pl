...
if ($DEBUG) {
    ...
    &LogDebug("$prog $ver starting in debug mode. Commandline options: %A");
}
...
Log(
"$allmachines computers in the database, $dbscans ($daypercent\%) reported "
. "in the last day, $dbscansweek ($weekpercent\%) reported within the week.\n"
);
Log(
"$allmachines_udd unmanaged devices in the database, $dbscans_udd "
. "($daypercent_udd\%) were seen in the last day, $dbscansweek_udd "
...
sub CullScanFiles {
 ...
 # If there was no UUID in the database, move along to the next line of the file
       ...
       # The first one to match wins.
                ...
                elsif ( $parts[0] =~ m/^Network - TCPIP - Host Name/x ) {
                    $marker  = &Trim( $parts[1] );
                    $newname = $SCANDIR . "\\_" . $marker . "_" . $source;
                    last;
                }
                elsif ( $parts[0] =~ m/^Network - TCPIP - Address/x ) {
                    $marker  = &Trim( $parts[1] );
                    $newname = $SCANDIR . "\\_" . $marker . "_" . $source;
                    last;
                }
...
# Note that rows with no IP address are useless.
            $sql =
"select IPADDRESS from UNMANAGEDNODES where XDDEXCEPTION='0' and "
...
# count missing patches, by severity: http://community.landesk.com/support/message/12139
    $sql =
"Select VulSeverity.DisplayName, count(computer.displayname) as '# of "
. "missing patches' FROM Scope INNER JOIN ScopeComputer ON "
. "Scope.Scope_Idn = ScopeComputer.Scope_Idn INNER JOIN Computer INNER JOIN "
. "ComputerVulnerability ON Computer.Computer_Idn = "
. "ComputerVulnerability.Computer_Idn INNER JOIN Vulnerability ON "
. "ComputerVulnerability.Vul_ID = Vulnerability.Vul_ID INNER JOIN "
. "VulSeverity ON Vulnerability.Severity = VulSeverity.Severity_ID ON "
. "ScopeComputer.Computer_Idn = Computer.Computer_Idn WHERE "
. "ComputerVulnerability.Detected = 1 GROUP BY VulSeverity.DisplayName "
...
"update UNMANAGEDNODES set MANUFACTURER=? where IPADDRESS=?";
        ...
        if ($DEBUG) {
            &LogDebug( "Set OS Name of " . $host->addr . " to " . $OS );
        }
        $goodcount++;
