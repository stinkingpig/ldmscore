...
# this SQL which looks like a good starting point (Jobs that haven't been
# started yet or didn't upgrade to 8.8 properly):
...
# ORDER By  TASK_NAME
# Chris Eales suggests (All jobs that are scheduled):
...
# TODO -- NO-NMAP mode. commandline and gui option to stop it.
# TODO -- generic on/off for features -- run a set of things without doing the
# others. what about a UI mode with buttons that do the things?
...
sub RenameFile {
    ...
    if ($DEBUG) {
        ...
        if ( copy( "$oldfilename", "$newfilename" ) ) {
            unlink($oldfilename)
              || &LogWarn("RenameFile: unlink $oldfilename failed: $!");
            $renamecount++;
            return 0;
        }
        else {
            &LogWarn(
                "RenameFile: copy $oldfilename to $newfilename failed: $!" );
        }
    }
    return 1;
}
...
sub CountForcedScans {
    my ( $handle, $base, %Event, $scanevent, $result );
    ...
    if ($result) {
        if ($DEBUG) {
            &LogDebug( "CountForcedScans found result of $result, "
                  . "record of $scanevent." );
        }
        return $result;
    }
...
# Need to limit this to a single day's data
sub CountHashErrors {
    my ( $handle, $base, %Event, $hashevent, $result );
    ...
    if ($result) {
        if ($DEBUG) {
            &LogDebug( "CountHashErrors found result of $result, "
                  . "record of $hashevent." );
        }
        return $result;
    }
...
sub CountSchemaErrors {
    my ( $handle, $base, %Event, $schemaevent, $result );
...
sub CullOrphanCompVulns {
    &OpenDB;
    my $sqlbase .= "from computervulnerability where computervulnerability_idn"
    ...
    if ($DEBUG) {
        &LogDebug("I would delete $comma{$orphancount} orphaned computer "
            . "vulnerability " . PL("record", $orphancount) . " from the "
            . "database."); 
    } else {
        &DoDBAction($sql);
...
sub CullProducts {
    ...
    # working around.
    $sql =
        "SELECT DISTINCT TOP 100 Product_Idn, "
...
sub SetupTask {
    ...
    if ( -e $script ) {
        ...
        print $FILE "[MACHINES]\n";
        print $FILE "REMEXEC00=$cli /map, SYNC\n";
        close $FILE;
    }
