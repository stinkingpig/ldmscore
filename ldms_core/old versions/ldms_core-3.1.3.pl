...
# TODO -- Progress indication after clicking Authorization button
...
my ( $osupdates, $macupdates, $vendorupdates, $udddeletes, $goodcount,
     $recentvulns ) = 0;
...
sub genfilename {
    my ( $sec, $min, $hour, $mday, $mon, $year, $wday, $yday, $isdst ) =
      localtime(time);
    my $return = sprintf "%04d%02d%02d-%02d%02d%02d", $year + 1900,
...
. "order by lastupdinvsvr asc)";
        ...
        if ($db_type eq "ORA") {
            $sql = 
"update TCP set address = '' where TCP.computer_idn = "
. "(select c.computer_idn from COMPUTER c "
. "join TCP t on t.computer_idn = c.computer_idn "
. "inner join TCP t1 on t.address = t1.address "
. "where t.address = ? "
. "and c.lastupdinvsvr=(select max(lastupdinvsvr) "
. "from COMPUTER))";
        }
       $sth = $dbh->prepare($sql)
            or &LogWarn("$sql caused $DBI::errstr");
        ...
        if ($newname) {
            if ($DEBUG) {
                ...
                if ( copy( "$file", "$newname" ) ) {
                    unlink($file) || &LogWarn("CullScanFiles: unlink $file: $!");
                }
                else {
...
sub ServiceCheck {
    ...
    if ($DEBUG) {
        &LogDebug("Checked $servicetarget, current state = $status{CurrentState}.");
    }
...
# we should check the status again before getting excited.
        ...
        if ($DEBUG) {
            &LogDebug(
                "Leaving ServiceCheck after first restart try was "
...
sub SetupTask {
    ...
    if ( -e $script ) {
        ...
        print $SCRIPTFILE "LOCEXEC=$cli, SYNC\n";
        close $SCRIPTFILE;
    }
    ...
    if ($count) {
		...
		$sql = "insert into LD_TASK_CONFIG "
