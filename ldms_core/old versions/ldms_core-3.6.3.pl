...
# TODO -- advanced values in inventory and custom job tabs.
...
# TODO -- Support proxy servers (will need for geo-location)
...
# gateway usage
# TODO -- actually delete older provisioning jobs instead of just hiding them;
# would parse the date in the name and use deletion days to select the targets
# TODO -- move setting defaults into the NSIS script instead of Perl.
# TODO -- are there crazy amounts of files in C:\\Windows\\Temp or 
# c:\\WINDOWS\\Microsoft.NET\\Framework\\v2.0.50727\\Temporary ASP.NET Files
# TODO -- purge .tmp files from ldlogon\VulnerabilityData
...
my (
    ...
    $ldmsrrdfile_rc,    $tasklife,            $tasks_percent,
    $pudcount,          $db_is_open
);
...
sub LocalSQL {
    ...
    if ( $hostname eq $db_instance ) {
        if ($DEBUG) { &LogDebug("Core is using a local SQL Server."); }
        return 1;
    }
    return 0;
}
...
sub CountRAM {
    ...
    if ( $SystemList->Count > 0 ) {
        foreach my $System ( in $SystemList) {
            ...
            if ($DEBUG) { &LogDebug("Core has $output MB of RAM."); }
        }
    }
        ...
        if ( $_ =~ /^multi/x ) {
            ...
            if ( chomp( $parts[0] ) eq chomp($defaultboot) ) {
                if ( $parts[1] =~ /\/PAE/xi ) {
                    $PAE = 1;
                    if ($DEBUG) {
                        &LogDebug("/PAE is enabled in $bootinifile");
                    }
                }
                if ( $parts[1] =~ /\/3GB/xi ) {
                    $TGB = 1;
                    if ($DEBUG) {
                        &LogDebug("/3GB is enabled in $bootinifile");
                    }
                }
            }
        }
...
sub DoTaskLife {
    ...
    if ($tasklife) {
        if ($DEBUG) { &LogDebug("Calculating how long tasks take"); }
...
"Scheduled Tasks and Policies enforced by LANDesk go undone an average of ";
        ...
        if ($taskminutes) {
            if (
                $taskmessage =~ m/days   # either days
                                   |hours # or hours
                               /x
              )
            {
...
sub GetTaskData {
    ...
    if ( $db_type eq "SQL" ) {
        ...
        $sql =
          ...
          . "average_duration_in_sec from ld_task t, ld_task_machine m where "
          . "ld_task.ld_task_idn = ld_task_machine.ld_task_idn "
          . "and ld_task_machine.mac_retcode <> 1110 "
          . "and ld_task_machine.mac_retcode <> 1101 "
          . "and ld_task_machine.mac_retcode <> 1001 "
          . "and ld_task_machine.status_time Is Not Null";
...
"from fileinfo where fileinfo_idn in ( select distinct a.fileinfo_idn From "
    ...
    if ($orphancount) {
        $sql = "delete " . $sqlbase;
        if ($DEBUG) {
            &LogDebug( "I would delete $comma{$orphancount} orphaned SLM "
                  . "product "
            ...
            &DoDBAction($sql);
            &Log(   "Deleted $comma{$orphancount} orphaned SLM "
                  . "product "
                  . PL( "record", $orphancount )
                  . " from the "
                  . "database." );
...
"and OSNAME is null or OSNAME='' or OSNAME='UNKNOWN' or OSNAME='UNIX' ";
        ...
        if ($DEBUG) {
            &LogDebug(
                "found " . $nmapcount . " NMAP targets, SQL string was $sql" );
        }
    }
