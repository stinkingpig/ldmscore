...
# TODO -- It's been 7 days or more since you downloaded content
...
# TODO -- The new alerting system might be a better way to look for sync scans
...
my ( $db_type, $db_user, $db_pass, $db_name, $db_instance, $dbreindex );
my ( $sql,        $dbh,      $sth );
...
my (
    ...
    $lbl_dbreindex,          $form_dbreindex,   $form_patchdir_override, 
...
my ( $osupdates, $macupdates, $vendorupdates, $udddeletes, $goodcount ) = 0;
my ( $patchcountsreport, $autofixreport, $patchurlsreport, $dupreport,
     $ALERTDIR,          $purgemarker,   $updatemessage );
...
if ( $A{dbreindex} )    { $dbreindex = $A{dbreindex}; }
if ( $A{m} )            { $mailto            = $A{m}; }
if ( $A{f} )            { $mailfrom          = $A{f}; }
...
Usage: $prog [-d] [-u] [-h] [-x=10] -db_type=[SQL|ORA] 
    ...
    -dbreindex   Allow reindexing of the database.
	...
	-update=0    Disable online checking for an updated version
...
sub CullAlerts {
    ...
    if (!-e $purgemarker ) {
        ...
        my $PM;
        open($PM, '>', "$purgemarker") 
            or &LogWarn("Can't create $purgemarker: $!");
        print $PM "\n";
        close($PM);
    ...
    if ($DEBUG) {
        &LogDebug( "I would purge "
            . $alertpurges . " alert messages from the database."
        );
    } else {
...
sub DBReindex {
        ...
        &LogWarn(
"Database Reindexing is still not safe for prime time. Skipping."
        );
        return 1;
    ...
    if ( $db_type eq "SQL" ) {
        ...
        $indexsql = "EXEC [sp_MSforeachtable] \@command1=\"DBCC DBREINDEX('?')\"";
...
sub ReadRegistry {
    ...
    if ($RegKey) {
        ...
        $STORAGEDIR = $PATCHDIR = $SCANDIR = $ldscan = $xddscan = $sdscan =
          $ALERTDIR = $purgemarker = Win32::GetShortPathName($ldmain);
        ...
        $sdscan      .= "sdstatus";
        $ALERTDIR    .= "alertqueue";
        $purgemarker .= "alertqueue\\purge.sig";
...
# LANDesk specific configuration if present
    ...
    if ($RegKey) {
        ...
        $dbreindex      = $RegKey->GetValue("dbreindex");
...
sub Setup {
    ...
    if ($DEBUG) {
        &LogDebug(
            "Wrote $db_type, $db_instance, $db_name, $db_user, "
              . "$db_pass_storage, $dbreindex, $mailserver, $mailfrom, "
...
sub Show_MainWindow {
    ...
    # Reindex question
    $lbl_dbreindex = $main->AddLabel(
        -name    => "lbldbreindex",
        -text    => "Enable Reindexing?",
        -pos     => [ 185, 125 ],
        -size    => [ 275, 20 ],
    );

    $form_dbreindex = $main->AddCheckbox(
        -name    => "form_dbreindex",
        -tabstop => 1,
        -pos     => [ 290, 123 ],
        -size    => [ 20, 20 ],
...
sub Default_Click {
    ...
    $dbreindex = $form_dbreindex->Checked();
...
"dbi:ODBC:driver={SQL Server};Server=$db_instance;Database=$db_name;UID=$db_user;PWD=$db_pass"
        ...
        if ($DEBUG) {
            &LogDebug(
               "Okay clicked in MainWindow: Opening database with "
    ...
    if ( $db_type eq "SQL" ) {
        $sql =
            "select top 1 host,replyemail from ld_task_smtp "
