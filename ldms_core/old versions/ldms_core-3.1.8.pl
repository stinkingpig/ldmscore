...
my ( $sql, $dbh, $sth );
my ( @rows,       @files,    @patchurls, @patchcounts, @autofixcounts );
my ( $mailserver, $mailfrom, $mailto,    $mailmessage, $sendemail );
my ( $mail_auth_user, $mail_auth_pass, $mail_auth_type, $mailverbosity );
...
# Setup UI variables
my (
    $DBWindow,               $lbl_Instructions,   $form_db_instance,
    $form_db_name,           $form_db_user,       $form_db_pass,
    $lbl_db_type,            $form_db_type,       $db_type_binary,
    $form_patchdir_override, $count,              $cfg_params,
    $btn_DBWindowDefault,    $btn_DBWindowCancel, $btn_DBWindowHelp,
    $btn_DBWindowDBInfo,     $btn_browsepatchdir, $btn_browsenmap,
    $DBWindowsb
);
my (
    $ConfigWindow,           $lbl_email,
    $lbl_patch,              $form_mailserver,
    $form_mailfrom,          $form_mailto,
    $form_deletiondays,      $btn_ConfigWindowdefault,
    $btn_ConfigWindowcancel, $ConfigWindowsb,
    $form_nmap,              $form_nmap_u,
    $form_nmap_options,      $form_nmap_ulabel,
    $btn_mailauth,           $btn_ConfigWindowHelp
);
my (
    ...
    $lbl_mail_auth_type,  $btn_mailtest,         $form_mailverbosity,
    $lbl_mailverbosity
);
my ( $w, $h, $ncw, $nch, $dw, $dh, $desk, $wx, $wy );
my (
    $ConfigWindoww,   $ConfigWindowh,  $ConfigWindowncw,
    $ConfigWindownch, $ConfigWindowwx, $ConfigWindowwy
);
my (
...
my (
    $devicename,      $allmachines,   @dupmachines,
    $dbscans,         $dbscansweek,   $allmachines_udd,
    $dbscans_udd,     $source,        $vulnlife,
    $dbscansweek_udd, @dupaddresses,  $daypercent,
    $weekpercent,     $forcedpercent, $forcedfullscans,
    $pkhasherrors,    $vulndays,      $daypercent_udd,
    $weekpercent_udd, $vulnhours,     $vulnminutes,
    $vulnseconds,     $vulnmessage,   $supercededvulncount,
    @supercededvulns, $hashpercent,   @dualboots,
    %rtn
);
my (
...
Usage: $prog [-d] [-u] [-h] [-x=10] -db_type=[SQL|ORA] 
    ...
    -v=1-5       verbosity threshold for email sending.
...
sub DoInventoryMath {
    ...
    if (@dupmachines) {
        ...
        if ($dupcount) {
            if ( $dupcount == 1 ) {
                $dupreport = "Duplicate computer record detected:\n" . $dupreport;
            }
            else {
...
sub ServiceCheck {
    ...
    if ( !defined($servicetarget) ) {
        &LogWarn(
            "ServiceCheck called without a service to check. " .
            "$servicetarget" );
        return 1;
    }
...
sub ConfigWindow_Show {
    ...
    $ConfigWindoww   = $leftmargin + $form_patchdir_override->Width() +
    $rightmargin + $ConfigWindowncw;
...
sub ConfigWindow_Resize {
    $ConfigWindowsb->Move( 0, $ConfigWindow->ScaleHeight - $ConfigWindowsb->Height );
    $ConfigWindowsb->Resize( $ConfigWindow->ScaleWidth, $ConfigWindowsb->Height );
    return 0;
}
