...
# TODO -- on/off for specific features -- run a set of things without doing the
...
EOD
        ...
        &LogWarn(
"Database Reindexing not supported for Oracle."
        );
        return 1;
...
sub ConfigWindow_Show {
    ...
    $form_patch_dolabel = $ConfigWindow->AddLabel(
        -name => "patch_dolabel",
        -text => "Should ldms_core purge patches that are no longer needed?",
        -pos  => [ $leftmargin - 80, $nexthoriz + 3 ],
        -size => [ 300, 20 ],
    );
    ...
    $lbl_dbreindex_do = $ConfigWindow->AddLabel(
        ...
        -text =>
          "Should ldms_core reindex your database?",
        -pos  => [ $leftmargin - 80, $nexthoriz + 3 ],
        -size => [ 290,              20 ],
    );
