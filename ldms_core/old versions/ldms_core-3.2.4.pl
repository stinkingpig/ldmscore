...
# TODO -- more efficient to pluralize with http://search.cpan.org/dist/Lingua-EN-Inflect/lib/Lingua/EN/Inflect.pm
...
sub CountSchemaErrors {
    ...
    if ($result) {
        &Log("CountSchemaErrors found $result schema insertion errors");
        return $result;
    }
    else {
        if ($DEBUG) {
