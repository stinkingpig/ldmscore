...
# scheduled task..
# TODO -- option not to check for automatic patch process
# TODO -- option not to cull IPs... duplicates could exist because of NAT,
# gateway usage
# TODO -- fix copy of .css, .ico, .png files ($dir isn't getting set)
# TODO -- fix warnings from deleting errorscan files
...
"$Event{Source} $Event{EventID} $Event{TimeGenerated}"
    ...
    if ($DEBUG) {
        &LogDebug( "ReadEventViewer reviewed $schemaevent "
              . " Inventory service " . PL( "record", $schemaevent )
...
sub CullRollingLog {
    ...
    while ( my $logfile = readdir($DIR) ) {
        ...
        next if $logfile =~ /^       # from the beginning of the line
                            \.\.?   # two dots then anything
                            $       # to the end of the line
                            /x;

        # Ignore the base logfile, we only want to get rolling logs.
        next if $logfile =~ /ldinv32\.exe\.log/ix;
        # Ignore non-log files
        if (
            $logfile =~ /^ldinv32\.exe # if it begins ldinv32.exe
          ...
          )
        {
            ...
            # stat, 7 is SIZE, 8 is ATIME, 9 is MTIME, 10 is CTIME
            my ($size, $ctime) = ( stat($ldmain."\\".$logfile) )[7,10]
                      or &LogWarn("CullRollingLog stat($logfile) failed: $!");
               if ($DEBUG) {
                    &LogDebug(
                        "stat() says $logfile ctime is $ctime, size is $size" );
               }
               if ( $ctime < (time() - 86400) ) {

                    $logcount++;
                    $logsize += $size;
                    &DeleteFile( $ldmain . "\\" . $logfile, $ctime );
               }
        }
    }
    if ($logcount) {
