...
my (
    ...
    $osupdates,    $macupdates,  $vendorupdates
);
...
# Note that rows with no IP address are useless.
            $sql =
"select DISTINCT IPADDRESS, LASTSCANTIME from UNMANAGEDNODES where "
. "XDDEXCEPTION='0' and IPADDRESS is not NULL ";
...
sub DoNMAP {
    ...
    if (@Address_np) {
        ...
        &change_balloon("tip",
            "Scanning Unmanaged Devices which don't answer ping"
        );
