...
$sql = "select distinct [computer].[devicename] from [computer] inner join [computer] t1 on [computer].[devicename] = t1.[devicename] where [computer].[computer_idn] <> t1.[computer_idn] order by [computer].[devicename] asc";
$sth = $dbh->prepare($sql) or &LogDie("$sql caused $DBI::errstr\n");
$sth->execute or &LogDie("$sql caused $DBI::errstr\n");
my @dupmachines;
while ( @rows = $sth->fetchrow_array() ) {
...
$sth = $dbh->prepare($sql) or &LogDie("$sql caused $DBI::errstr\n");
$sth->execute or &LogDie("$sql caused $DBI::errstr\n");
my $allmachines_udd = $sth->fetchrow();
...
$sth = $dbh->prepare($sql) or &LogDie("$sql caused $DBI::errstr\n");
$sth->execute or &LogDie("$sql caused $DBI::errstr\n");
my $dbscansweek_udd = $sth->fetchrow();

# What's the average lifespan of a detected vulnerability?
if ( $db_type eq "SQL" ) {
	$sql = "select avg(datediff(ss,datedetected,patchinstalldate)) from computervulnerability where patchinstallstatus = 'Done' and datedetected is not null and patchinstalldate is not null";
}
else {
    # Oracle Support
	$sql = "select avg((patchinstalldate - datedetected) * 24 * 60 * 60) from computervulnerability where patchinstallstatus = 'Done' and datedetected is not null and patchinstalldate is not null";
}
$sth = $dbh->prepare($sql) or &LogDie("$sql caused $DBI::errstr\n");
$sth->execute or &LogDie("$sql caused $DBI::errstr\n");
my $vulnlife = $sth->fetchrow();
...
my ($vulndays,$vulnhours,$vulnminutes,$vulnseconds) = &ConvertSeconds($vulnlife);
if ( $vulndays > 50 ) {
	$sendemail = 1;
}
...
Log(
"$allmachines computers in the database, $dbscans ($daypercent\%) reported in the last day, $dbscansweek ($weekpercent\%) reported within the week. $allmachines_udd unmanaged devices in the database, $dbscans_udd ($daypercent_udd\%) were seen in the last day, $dbscansweek_udd ($weekpercent_udd\%) were seen within the week. Vulnerabilities go unpatched an average of $vulndays days, $vulnhours hours, and $vulnminutes minutes."
);
...
EOD
	    ...
	    &LogDie("Database Reindexing cannot continue, Database type is not specified!\n");
	}

	Win32::GUI::DoEvents() >= 0 or &LogDie("Killed by user.");
	$sth = $dbh->prepare($indexsql) or &LogDie("Database reindexing caused $DBI::errstr\n");
...
sub CountPendingScans() {
	...
	Win32::GUI::DoEvents() >= 0 or &LogDie("Killed by user.");
	if (-e $ldscan) {
	    opendir( DIR, "$ldscan" ) or &LogDie("Can't open directory $ldscan: $!\n");
    	$scancount = 0;
...
"There are more than 200 inventory scans pending database insertion. You should investigate database performance."
	...
	if (-e $xddscan) {
	    opendir( DIR, "$xddscan" ) or &LogDie("Can't open directory $xddscan: $!\n");
    	$xddcount = 0;
	    while ( my $source = readdir(DIR) ) {
...
"There are more than 200 extended device discovery scans pending database insertion. You should investigate $ldmain\\XDDFiles2DB.exe.log."
	...
	if (-e $sdscan) {
	    opendir( DIR, "$sdscan" ) or &LogDie("Can't open directory $sdscan: $!\n");
    	$sdcount = 0;
	    while ( my $source = readdir(DIR) ) {
	            ...
	            if ( -w $file ) {
	                ...
	                if ( $atime < $time ) {
   	                 ...
   	                 if ($DEBUG) {
	                        ...
	                        Log(
"DEBUG: $patch is $days days old and no computers need it, so it should be deleted.\n"
	                        );
	                    }
	                    else {
...
sub CullScanFiles() {
	$trashcount  = 0;
	$renamecount = 0;
	opendir( DIR, "$SCANDIR" ) or &LogDie("Can't open directory $SCANDIR: $!\n");
	while ( my $source = readdir(DIR) ) {
	...
	if ( -e $STORAGEDIR ) {
	    $compresscount = 0;
	    my @filestokill;
	    opendir( DIR, "$STORAGEDIR" ) or LogDie("Can't open directory $STORAGEDIR: $!\n");
