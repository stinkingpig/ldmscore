...
# TODO -- Check scheduled tasks and policies for RRD stats
...
my (
    ...
    $np,             $nmap_unidentified,   @Address,
    $allmachines,    @dupmachines,         $dbscans,
    $dbscansweek,    $allmachines_udd,     $dbscans_udd,
    $source,         $vulnlife,            $dbscansweek_udd,
    @dupaddresses,   $daypercent,          $weekpercent,
    $daypercent_udd, %rtn,                 $weekpercent_udd,
    @dualboots,      $supercededvulncount, @supercededvulns,
    $update,         $lpmdir,              $emailsubject,
    $CullVulnsAggression
);
...
my (
    ...
    $form_showsystray,         $lbl_showsystray,
...
my $dir = Win32::GetShortPathName( $PROGRAMFILES . "/Monkeynoodle/ldms_core" );
if ( !-e $dir ) {
    $dir = Win32::GetShortPathName("C:/Program Files/Monkeynoodle/ldms_core");
}
...
$emailsubject = "$prog $ver output report";
...
sub CullProducts {
    ...
    foreach my $product (@tokill) {
        if ($DEBUG) {
            ...
            # This evaluation isn't working yet; it's still claiming that it
...
sub TopologyMap {
    ...
    if (@Networks) {
        ...
        foreach my $net (@Networks) {
            ...
            my $label= "$gateway is $hops " . PL("hop",$hops) . " away "
                      . " and has $size managed "
                      . PL( "node", $size )
                      . " behind it." ;
...
<html>
	<head>
		<title>ldms_core Latest Network Map</title>
		<meta http-equiv="pragma" content="no-cache" />
		<meta http-equiv="cache-control" content="no-cache" />
		<meta http-equiv="generator" content="$prog $ver" />
		<meta http-equiv="content-type" content="text/html; charset=iso-8859-1" />
...
href="http://www.droppedpackets.org/scripts/ldms_core">$prog $ver</a> using <A HREF="http://bloodgate.com/perl/graph/manual/index.html">Graph::Easy</A>.
<!-- End Footer Block -->
	</body>
</html>
