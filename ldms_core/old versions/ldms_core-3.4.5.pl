...
# in TopologyMap, support a floor node count
...
sub CullPatches {
    ...
    if ($deletiondays) {
        ...
        # Get the patch names
        $sql = "select top 100 patch from computervulnerability where "
          . "detected=0 and patch != '*' order by lastscandate";
...
EOHTML
    ...
    if ($mapfloor) { 
        $output .= "<p>Routers with fewer than $mapfloor " 
        . PL("node",$mapfloor) . " behind them were not included in this "
        . "topology map.</p>"; 
    }
    $output .= &GenerateWebFooter;
    open( $FILE, '>', "$graphfile" )
