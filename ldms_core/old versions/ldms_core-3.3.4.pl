...
# TODO -- Check scheduled tasks and policies for RRD stats -- Zubrowski posted
# this SQL which looks like a good starting point:
# SELECT     *, REMOTE_OBJECT_NAME AS Expr1
# FROM       LD_TASK
# WHERE     (DELIVERYMETHOD_IDN <> 0) AND ((REMOTE_OBJECT_NAME IS NULL) OR (REMOTE_OBJECT_NAME = ' ') OR (REMOTE_OBJECT_HASH IS NULL) OR (REMOTE_OBJECT_HASH = ' '))
# ORDER By  TASK_NAME
# Chris Eales suggests:
#SELECT DISTINCT LD_TASK.TASK_NAME, LD_TASK.NEXT_START, ConsoleUser.UserName
#FROM         LD_TASK INNER JOIN
#                      ConsoleUser ON LD_TASK.CONSOLEUSER_IDN = ConsoleUser.ConsoleUser_Idn
#WHERE     (LD_TASK.NEXT_START > GETDATE())
#ORDER BY LD_TASK.NEXT_START DESC
#
# TODO -- plot non-RFC1918 addresses on a map: http://search.cpan.org/dist/Geo-Google-StaticMaps/lib/Geo/Google/StaticMaps.pm
...
# TODO -- If MIME email were around, I could backup the certs by emailing
# them to the user every month or so.
...
# html.
# TODO -- Split setup mode into a separate binary?
# TODO -- allow LDMS-only users to suppress security messages
...
my (
    ...
    $form_showsystray,         $lbl_cullvulnsaggression,
    $lbl_cullvulnsaggression2, $form_cullvulnsaggression,
    $w,                        $h,
    $ncw,                      $nch,
    $dw,                       $dh,
    $desk,                     $wx,
...
sub DoDBAction {
    if ($dbh) {
        ...
        if ($@) {
            ...
            if ( defined($DBI::errstr) ) { $errormessage .= $DBI::errstr; }
            &LogWarn($errormessage);
            eval { $dbh->rollback() };
            if ($@) { &LogWarn("Couldn't roll back transaction"); }
            $dbh->{RaiseError} = 0;
            $dbh->{AutoCommit} = 1;
            return 1;
        }
...
sub RenameFile {
    my ( $base, $file, $newname ) = @_;
    $newname = $base . "\\_" . $newname . "_" . $file;
    my $oldname = $base . "\\" . $file;
    if ($DEBUG) {
        &LogDebug("RenameFile: I would be renaming $file to $newname");
    }
    else {
        if ( copy( "$oldname", "$newname" ) ) {
            unlink($oldname)
              || &LogWarn("RenameFile: unlink $oldname failed: $!");
            $renamecount++;
            return 0;
        }
        else {
            &LogWarn("RenameFile: copy $oldname to $newname failed: $!");
        }
    }
    return 1;
}
...
sub DoInventoryMath {
    ...
    if ($schemaerrors) {
        ...
        &Log(   "$schemaerrors Database schema "
              . PL( "error", $schemaerrors )
    ...
    if ($allmachines) {
        ...
        if ($dbscans) {
            ...
            if ($pkhasherrors) {
                ...
                &Log(   "$pkhasherrors Public Key hash "
    ...
    if (@dualboots) {
        ...
        if ($dupcount) {
            $dupreport =
              ...
              . " detected:\n"
              . $dupreport;
            &SetSeverity( 5, "Dual booting machines" );
            &Log("$dupcount $dupreport");
        }
    }
    return 0;
...
sub ReportLDMSStats {
    ...
    my $ldmsmessage = "$allmachines " . PL("computer",$allmachines) 
        . " in the database.";
    if ($dbscans) {
...
sub CountForcedScans {
    my ( $handle, $base, %Event, $event, $result );
    ...
    if ($result) {
        if ($DEBUG) {
            &LogDebug(
                "CountForcedScans found result of $result, record of $event.");
        }
        return $result;
    }
    else {
...
sub CountHashErrors {
    ...
    if ($result) {
        if ($DEBUG) {
            &LogDebug(
                "CountHashErrors found result of $result, record of $event.");
        }
        return $result;
    }
    else {
...
sub CountSchemaErrors {
    ...
    if ($result) {
        &Log(   "CountSchemaErrors found $result schema insertion "
              . PL( "error", $result )
              . ":" );
        return $result;
    }
...
sub CullPatches {
    ...
    if ($deletiondays) {
        ...
        # Get the patch count
        $sql = "select count(patch) from computervulnerability where "
          . "detected=0 and patch != '*'";
        $patchcount = &GetSingleString($sql);
    ...
    if ( $trashcount > 0 ) {
        if ($DEBUG) {
            &LogDebug( "CullPatches: patchcount is $patchcount, "
...
# Core now has two machines with address 1.
    ...
    if (@dupaddresses) {
        ...
        if ( $deadaddrcount > 0 ) {
            &Log(   "Cleared $deadaddrcount dead IP "
...
sub CullProducts {
    ...
    foreach my $product (@tokill) {
        if ($DEBUG) {
            ...
            if ( &DoDBAction($sql) ) {
                $killcount--;
                if ($DEBUG) {
                    &LogDebug( "Product deletion transaction failed "
    ...
    if ($killcount) {
        if ($DEBUG) {
            ...
            &Log(   "Deleted $killcount automatically discovered "
                  . PL( "product", $killcount )
                  . " from the database." );
...
sub CullScanFiles {
    ...
    closedir($DIR);
    if ( $trashcount > 0 ) {
        &Log( "Deleted $trashcount scan " . PL( "file", $trashcount ) );
    }
    if ( $renamecount > 0 ) {
        &Log( "Renamed $renamecount scan " . PL( "file", $renamecount ) );
    }
...
sub CompressStorageFiles {
    ...
    if ( $compresscount > 0 ) {
        &Log( "Compressed and deleted $compresscount stored scan "
...
sub TopologyMap {
    ...
    $sql = "select defgtwyaddr,count(address) from tcp where "
      . "nullif(address,'') is not null group by defgtwyaddr";
    ...
    if (@Networks) {
        ...
        foreach my $net (@Networks) {
            ...
            # If the gateway isn't pingable, it should still be drawn on the
            # map; but we'll draw it as "One hop past the maximum possible"
            my $hops = 31;
            ...
            my $node        = $graph->add_node($gateway);
...
sub SetDefaults {
    ...
    $dir = Win32::GetShortPathName( $PROGRAMFILES . "/Monkeynoodle/ldms_core" );
    if ( !-e $dir ) {
        $dir =
...
<html>
	<head>
		...
		<meta http-equiv="generator" content="$prog $VERSION" />
		<meta http-equiv="date" content="$time" />
		<meta http-equiv="content-type" content="text/html; charset=iso-8859-1" />
        <link HREF="ldms_core.css" rel="stylesheet" type="text/css"> 
        <link REL="SHORTCUT ICON" HREF="ldms_core.ico">
	</head>
...
sub ConfigWindow_Show {
    ...
    $form_nmap_u = $ConfigWindow->AddCheckbox(
        ...
        -pos     => [ $form_nmap_ulabel->Width(), $nexthoriz - 3 ],
        -size    => [ 20, 20 ],
    );
    ...
    $form_cullvulnsaggression->SetBuddy( 0,
        $ConfigWindow->AddLabel( -text => "Move partially superceded" ) );
    $form_cullvulnsaggression->SetBuddy( 1,
        $ConfigWindow->AddLabel( -text => "Don't move any vulns" ) );

    $lbl_cullvulnsaggression2 = $ConfigWindow->AddLabel(
        -name    => "lbl_cullvulnsaggression2",
        -text    => "Move completely superceded",
        -tabstop => 0,
        -pos     => [ 150, $nexthoriz += 25 ],
        -size => [ 250, 20 ],
    );
    ...
    $form_update->SetBuddy( 0, $ConfigWindow->AddLabel( -text => "Weekly" ) );
    $form_update->SetBuddy( 1, $ConfigWindow->AddLabel( -text => "Never" ) );

    # End update frequency slider rows

    # Begin showsystray row (label and checkbox)
    $form_showsystray = $ConfigWindow->AddLabel(
    ...
    $form_showsystray = $ConfigWindow->AddCheckbox(
        ...
        -tabstop => 1,
        -checked => $showsystray,
        -pos     => [ $form_showsystray->Width(), $nexthoriz - 3 ],
        -size    => [ 20, 20 ],
    );
