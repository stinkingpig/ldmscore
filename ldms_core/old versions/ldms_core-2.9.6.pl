...
my ( $sql,     $DSN,     $dbh,     $sth );
my ( @rows,    %nodes,   @files,   $key,  $value, @patchurls, @patchcounts );
...
my ( $source, $vulnlife, @dupaddresses );
my ( $daypercent, $weekpercent, $forcedpercent, $forcedfullscans );
my $pkhasherrors;
my ( $daypercent_udd, $weekpercent_udd );
...
Log("$allmachines computers in the database, $dbscans ($daypercent\%) reported in
the last day, $dbscansweek ($weekpercent\%) reported within the week.\n\n");
...
if (@patchcounts) {
    our $patchcountsreport = "Detected vulnerability counts by severity:\n";
...
sub ConvertSeconds {
	...
	if ($DEBUG) { Log("ConvertSeconds returned $days, $hours, $minutes, $seconds"); }
...
sub DoInventoryMath {
	...
	if ($dbscans) {
        ...
        if ($DEBUG) { &Log("$pkhasherrors Public Key hash errors in the last day."); }
		# Rescan forced?
		$forcedfullscans = &CountForcedScans();
        if ($DEBUG) { &Log("$forcedfullscans forced full scans in the last day."); }
	...
	if (@dupaddresses) {
	    our $dupreport = "Duplicate IP Addresses detected:\n";
...
sub DoPatchMath {
	...
	if ($vulnlife) {
		if ($DEBUG) { &Log("Calculating how long vulns live"); }
...
"DEBUG: $patch is $deldays days old and no computers need it, so it should be deleted.\n"
        ...
        if ( $trashcount > 0 ) {
			if ($DEBUG) { &Log("CullPatches: trashcount is $trashcount, totalsize is $totalsize"); }
...
"and OSNAME is null or OSNAME='' or OSNAME='UNKNOWN' or OSNAME='UNIX' ";
    ...
    if ($nmapcount) {
        ...
        if ($pingcount) {
            ...
            Log(
"Scanned " . $pingcount . " unmanaged nodes without OS Names which responded to ping."
            );
            Log(
"Finished NMAP scanning ping-friendly unmanaged nodes in the database. There were $goodcount successful scans and $badcount failed scans."
        ...
        if (@Address_np) {
            ...
            $badcount  = 0;
            Log(
"Scanning " . $nopingcount . " unmanaged nodes without OS Names which don't respond to ping. This may take a significant amount of time to complete."
            );
...
"select patch.comments from patch where comments LIKE '%http%' and download='0' and vulnerability_idn in (select distinct vulnerability.vulnerability_idn from vulnerability inner join computervulnerability t1 on vulnerability.vul_id = t1.vul_id where t1.detected='1' and vulnerability.type='0' and vulnerability.fixable='3')";
    ...
    $sql = "select distinct computer.devicename from computer inner join 
    computer t1 on computer.devicename = t1.devicename where 
    computer.computer_idn <> t1.computer_idn order by computer.devicename asc";
    $sth = $dbh->prepare($sql) or &LogDie("$sql caused $DBI::errstr\n");
    $sth->execute or &LogDie("$sql caused $DBI::errstr\n");
    while ( @rows = $sth->fetchrow_array() ) {
    ...
    $sql = "select distinct tcp.address from tcp inner join tcp t1 on
    tcp.address = t1.address where tcp.computer_idn <> t1.computer_idn order
    by tcp.address asc";
    $sth = $dbh->prepare($sql) or &LogDie("$sql caused $DBI::errstr\n");
    $sth->execute or &LogDie("$sql caused $DBI::errstr\n");
    while ( @rows = $sth->fetchrow_array() ) {
...
sub Help_Click {
    if ($DEBUG) { Log("DEBUG: Help clicked in Main Window"); }
    open_browser(
...
sub secondHelp_Click {
    if ($DEBUG) { Log("DEBUG: Help clicked in SecondWindow"); }
    open_browser(
'http://www.droppedpackets.org/scripts/ldms_core/ldms_core-manual'
    );
...
sub show_mailauth {
    ...
    if ($DEBUG) { 
        Log("DEBUG: $mailserver supports auth types: $auth_types"); 
    }
    ...
    if ($auth_types) {
        ...
        Win32::GUI::MessageBox(
            0,
            "I can't get a list of authentication types from that server --
            are you sure about its name?",
            "Not so ready yet", 32
        );
        return 0;
    }
    ...
    foreach my $auth_type (@mail_auth_options) {
        ...
        if ($DEBUG) { 
            Log("DEBUG: mailauth window added auth type: $auth_type"); 
        }
    }
...
sub mailauthDefault_Click {
    ...
    if ($DEBUG) { Log("DEBUG: Okay clicked in MailAuthWindow"); }
    ...
    if ($DEBUG) { 
        Log("DEBUG: read $mail_auth_user, mail_auth_pass, $mail_auth_type from user input"); 
    }
    $mailauth->Hide();
    return 0;
}
...
sub mailauthCancel_Click {
    if ($DEBUG) { Log("DEBUG: Cancel clicked in MailAuthWindow"); }
    $mailauth->Hide();
    return 0;
}

sub mailauthHelp_Click {
    if ($DEBUG) { Log("DEBUG: Help clicked in MailAuthWindow"); }
    open_browser(
'http://www.droppedpackets.org/scripts/ldms_core/ldms_core-manual'
    );
