...
# TODO -- Send a specific email for each warning
...
use Net::Ping;
use Sys::Hostname;
use Lingua::EN::Inflect qw(PL NUMWORDS);
...
if ( $sendemail <= $mailverbosity || $DEBUG ) {
    if ($DEBUG) {
        ...
        &LogDebug("Sending email report to $mailto.");
        &SendEmail;
    }
    if ( $sendemail != 0 ) {
        &ChangeBalloon( "tip", "Sending email" );
    ...
    if ($allmachines) {
        ...
        if ($dbscans) {
            ...
            if ($pkhasherrors) {
                ...
                if ( $hashpercent > 10 ) { &SetSeverity(2); }
            ...
            if ($forcedfullscans) {
                ...
                if ( $forcedpercent > 10 ) { &SetSEverity(2); }
                &Log(
...
sub CullProducts {
    ...
    foreach my $product (@tokill) {
        if ($DEBUG) {
            ...
            # First I have to get the file info instance; it does work to
            # delete directly, but it throws a referential integrity error, so
            # it's best to use some more cycles and memory here.
            $sql =
            ...
            $sql = "DELETE FROM PRODUCTFILE WHERE Product_idn='$product'";
            &DoDBAction($sql);

            foreach my $file_idn (@file_idn) {
                $sql = "DELETE FROM FILEINFO WHERE FileInfo_Idn='$file_idn'";
                &DoDBAction($sql);
            }
...
sub nmap_read_results {
    ...
    &ChangeBalloon( "tip", "Processing $hostaddr" );
...
sub ConfigWindow_Show {
    ...
    $form_showsystray = $ConfigWindow->AddLabel(
        ...
        -text => "Should ldms_core display activity with a system tray icon?",
        -pos  => [ 5, $nexthoriz += 25 ],
        -size => [ 290, 20 ],
    );
