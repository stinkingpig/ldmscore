...
if ($db_type eq "SQL") {
	$sql="select UNMANAGEDNODES_IDN,IPADDRESS from UNMANAGEDNODES where XDDEXCEPTION='0' and OSNAME is null or OSNAME='' or OSNAME='UNKNOWN' or OSNAME='UNIX' order by NEWID()";
} elsif ($db_type eq "ORA") {
	$sql="select UNMANAGEDNODES_IDN,IPADDRESS from UNMANAGEDNODES where XDDEXCEPTION='0' and OSNAME is null or OSNAME='' or OSNAME='UNKNOWN' or OSNAME='UNIX' order by DBMS_RANDOM.VALUE";
} else {
	croak "Cannot choose a select statement, Database type is not specified!\n";
}
